/* npy-mapper | version: 161230 | license: GPLv3 | author: Inpyo Jeon | website: http://npy.tools
 * sample image was designed by Edwin Husein, https://pixabay.com/en/users/JuralMin-2051452, CC0 */
...
function generateCode() {
	...
	for (i=0;i<cnt;i++) {
		...
		// 마크업 루프 생성
		gcAloop += '&nbsp;&nbsp;&nbsp;&nbsp;&lt;a href="' + link + '" style="position:absolute; top:' + top + unit + '; left:' + left + unit + '; width:' + width + unit + '; height:' + height + unit + '; display:block; background:url(about:blank);" target="' + target + '"&gt;&lt;/a&gt;' + '<br>';
...
function keyEvents(e) {
	switch (e.keyCode) {
		case 13: // Enter 키
			if ($('#pop-url').css('display') == 'block') $('#pop-url .pop-btn-confirm').trigger('click');
		case 27: // ESC 키
...
function recalcElMap() {
	...
	mapEl[recalcIndex][0][0] = (unit == 'px') ? parseInt($('.grid-box._active').css('left')) : parseInt($('.grid-box._active').css('left')) / imgWidth * 100;
	mapEl[recalcIndex][0][1] = (unit == 'px') ? parseInt($('.grid-box._active').css('top')) : parseInt($('.grid-box._active').css('top')) / imgHeight * 100;
	mapEl[recalcIndex][1][0] = (unit == 'px') ? parseInt($('.grid-box._active').css('left')) + $('.grid-box._active').outerWidth() : (parseInt($('.grid-box._active').css('left')) + $('.grid-box._active').outerWidth()) / imgWidth * 100;
	mapEl[recalcIndex][1][1] = (unit == 'px') ? parseInt($('.grid-box._active').css('top')) + $('.grid-box._active').outerHeight() : (parseInt($('.grid-box._active').css('top')) + $('.grid-box._active').outerHeight()) / imgHeight * 100;
}